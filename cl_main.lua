	...
	function StartDriveTest()
		...
		local plate = "ROADTST" .. math.random(1111, 9999)
	...
	CreateThread(function()
		-- Starter Ped
		local startmodel = `CSB_TrafficWarden`
	...
	function StopTheoryTest(success)
		...
		if success then
			QBCore.Functions.Notify("Passed Theory Test, Start your practical test!", "success", 5000)
			TriggerServerEvent('gg-drivingschool:server:passTheory')
			StartDriveTest()
		else
			QBCore.Functions.Notify("Failed Theory Test", "error")
			TriggerServerEvent('gg-drivingschool:server:setCooldown')
		end
	end
	...
	RegisterNetEvent('gg-drivingschool:paymentSuccess', function(skipTheory)
		if skipTheory then
			StartDriveTest()
		else
			StartTheoryTest()
	...
	RegisterNetEvent('drivingtest:client:reprint', function()
		lib.callback.await('gg-drivingschool:server:ReprintLicense')
	end)
	...
	CreateThread(function()
		...
		local reprintPrice = math.floor(Config.TestCost * 0.75) 
		local reprintLabel = "Reprint License ($" .. reprintPrice .. ")"

		-- Logic to hide reprint if player already has physical item
		local function canReprint()
			local pData = QBCore.Functions.GetPlayerData()
			local hasMetadata = pData.metadata and pData.metadata["licences"] and pData.metadata["licences"]["driver"]
			local hasItem = false

			if Config.Inventory == 'ox' then
				hasItem = exports.ox_inventory:Search('count', 'driver_license') >= 1
			else
				if pData.items then
			...
			return hasMetadata and not hasItem
		end
		...
		if Config.Target == 'qb' then
			exports['qb-target']:AddTargetEntity(START_PED, {
				options = {
					...
					{
						type = "client",
						event = "drivingtest:client:reprint",
						icon = "fas fa-id-card",
						label = reprintLabel,
						canInteract = function() return canReprint() end,
					},
				},
				distance = 2.0
			})
		...
		elseif Config.Target == 'interact' then 
			exports.interact:AddLocalEntityInteraction({
				...
				options = {
					{
						label = 'Start Driving Test',
						action = function() TriggerEvent('drivingtest:start') end,
					},
					{
						label = reprintLabel,
						action = function() TriggerEvent('drivingtest:client:reprint') end,
						canInteract = function() return canReprint() end,
					},
				}
			})
